...
void draw(){
  ...
  for(int i=0;i<pt.size()-1;i++){
    ...
    if(yellow_part1 && p2.y-p.y>0) yellow_rightdown+=sqrt(dpx*dpx+dpy*dpy);
    //ellipse(p.x,p.y,10,10);
    if(green_part1==false && rightdown>150)green_part1=true;
    else if(yellow_part1==true && green_rightup>150)green_ok=true;
    if(yellow_part1==false && rightdown>150)yellow_part1=true;
    else if(yellow_part1==true && green_rightup>150)yellow_ok=true;
  }
  println(rightdown);
